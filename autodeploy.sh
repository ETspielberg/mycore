...
eval "$(git for-each-ref --format='git push origin --delete %(refname)' refs/remotes/origin|grep -v $(echo "$(seq $FIRST $TRAVIS_BUILD_ID)" |sed -e 's|\(.*\)|refs/remotes/origin/\1|g'|xargs -I repl echo -n repl"\\|" && echo -n 'refs/remotes/origin/HEAD\|refs/remotes/origin/master')|sed -e 's|refs/remotes/origin/||')"
